  ...
  if (!isInitialized && authState.isLoading) {
    return (
      <div className="flex items-center space-x-3 bg-gray-50 p-4 rounded-lg">
        <div className="w-6 h-6 border-2 border-gray-300 border-t-apple-red rounded-full animate-spin"></div>
        <span className="text-gray-600">Apple Music 초기화 중...</span>
      </div>
    );
  }
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-red-800 font-medium">Apple Music 초기화 실패</h3>
            <p className="text-red-600 text-sm mt-1">{authState.error}</p>
          </div>
        </div>
        <button
          onClick={() => initialize()}
      ...
      <div className="bg-gradient-to-r from-pink-50 to-rose-50 border border-pink-200 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-gray-800 font-medium">Apple Music 연결됨</h3>
          ...
          <button
            ...
            className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            로그아웃
          </button>
        </div>
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6">
      <div className="text-center">
        ...
        <button
          ...
          className="bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white px-8 py-3 rounded-lg font-medium transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:transform-none flex items-center space-x-2 mx-auto"
        >
          {authState.isLoading ? (
            <>
